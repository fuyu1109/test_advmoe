...
def mart_trainer_v2(model, router, device, train_loader, epoch, optimizer, router_optimizer, args):
    ...
    for batch_idx, (images, labels) in enumerate(train_loader):
        ...
        # print('images11:',images)
        # print('labels11:',labels)
        x_adv, output_target, output_natural, count = earlystop(model, images, labels, step_size=args.step_size,
                                                                epsilon=args.epsilon, perturb_steps=args.num_steps,
                                                                tau=args.tau,
                                                                randominit_type="normal_distribution_randominit",
                                                                loss_fn='cent', rand_init=args.rand_init,
                                                                omega=args.omega)
        scores = model.router(images)  # 干净样本的路由器输出
        criterion_kl = torch.nn.KLDivLoss(reduction="sum")
        # 生成对抗样本
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # )
        #result_sub_dir = 'result/MART-ciafr10-lr0.008-noeval'
        save_checkpoint(
            {
                "epoch": epoch + 1,
                "arch": args.arch,
